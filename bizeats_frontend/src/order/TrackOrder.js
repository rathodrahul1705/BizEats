...
// Coordinates: Thane = start (cloud kitchen), Kalwa = end (customer)
const deliveryStart = [19.196061448334195, 72.95428775304241]; // Thane
const deliveryEnd = [19.20705360826353, 73.0111160893804];     // Kalwa
...
const customerIcon = L.divIcon({
  ...
  html: `
    <div class="custom-icon-wrapper">
      <svg width="32" height="32" viewBox="0 0 24 24" fill="#e65c00" xmlns="http://www.w3.org/2000/svg">
...
const kitchenIcon = L.divIcon({
  ...
  html: `
    <div class="custom-icon-wrapper">
      <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="#e65c00" viewBox="0 0 24 24">
        <path d="M6 19h12a4 4 0 0 0 0-8 4.5 4.5 0 0 0-8.64-1.35A3.5 3.5 0 0 0 6 13a3 3 0 0 0 0 6Zm0-2a1 1 0 1 1 0-2h1v-1.5a1.5 1.5 0 0 1 3 0V15h2v-2a2 2 0 1 1 4 0v2h2a2 2 0 1 1 0 4H6Z"/>
      </svg>
    </div>
  `,
  iconSize: [32, 32],
...
function RoutingWithLiveBike({ from, to }) {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    const bikeIcon = L.divIcon({
      ...
      html: `<div class="map-bike-icon">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#e65c00">
                <path d="M5.5 16a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5Zm13 0a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5ZM5.5 18A4.5 4.5 0 1 1 10 13.5H8.8a3.2 3.2 0 1 0-6.1 1.6l1.2.4c-.1-.2-.1-.4-.1-.5a2.5 2.5 0 0 1 5 0A2.5 2.5 0 0 1 5.5 18Zm13 0A4.5 4.5 0 1 1 23 13.5 4.5 4.5 0 0 1 18.5 18ZM17 6V5a1 1 0 0 0-1-1h-2v2h1.1l.4 1H11v2h1.4l-1.4 4.1-2-6.1H7v2h.6l3.3 9h1.9L16.7 9H18V7h-1.3l-.6-1Z"/>
              </svg>
            </div>`,
      iconSize: [30, 30],
      iconAnchor: [15, 15],
      ...
      .on("routesfound", function (e) {
        ...
        intervalRef.current = setInterval(() => {
          if (index < coordinates.length) {
            bikeMarker.setLatLng(coordinates[index]);
            index++;
          } else {
...
const TrackOrder = ({ user }) => {
  ...
  if (!selectedOrder) return <StripeLoader />;
  ...
  <div className="order-summary-card">
    ...
    <div className="track-map-container">
      <MapContainer
        center={[(deliveryStart[0] + deliveryEnd[0]) / 2, (deliveryStart[1] + deliveryEnd[1]) / 2]}
        zoom={13}
        scrollWheelZoom={false}
        className="delivery-map"
      >
        <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
        <Marker position={deliveryStart} icon={kitchenIcon} />
        <Marker position={deliveryEnd} icon={customerIcon} />
        <RoutingWithLiveBike from={deliveryStart} to={deliveryEnd} />
      </MapContainer>
    </div>
